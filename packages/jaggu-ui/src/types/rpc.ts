...
export type WebviewToExtensionMessage =
  | {
      type: 'user.submit';
      payload: { id: string; text: string; timestamp: number; attachedFiles?: string[] };
    }
  ...
  | { type: 'workspace.request_files'; payload?: Record<string, never> }
  | { type: 'terminal.clear'; payload?: Record<string, never> }
  ...
  | {
      ...
      payload: { editSetId: string; approvedFiles?: string[]; rejectedFiles?: string[] };
    }
...
export type ExtensionToWebviewMessage =
  ...
  | {
      type: 'agent.config';
      payload: { provider: string; model: string; models?: ModelDescriptor[] };
    }
  | {
      type: 'model.health_changed';
      payload: { modelId: string; health: ModelHealthStatus; detail?: string };
    }
  | {
      type: 'model.routed';
      payload: {
  ...
  | {
      ...
      payload: { proposalId: string; filePath: string; diffSummary: string; timestamp: number };
    }
  | {
  ...
  | {
      ...
      payload: { taskId: string; unplannedFiles: string[]; reason: string };
    }
  ...
  | { type: 'workspace.files'; payload: { files: string[] } }
  | {
      type: 'terminal.log';
      payload: {
