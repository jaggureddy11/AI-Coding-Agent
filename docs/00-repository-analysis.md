...
This document establishes the technical foundation for building **JAGGU**, a Cursor-class AI-native coding agent developed on top of the open-source Visual Studio Code (`microsoft/vscode`) ecosystem.

Rather than executing a naive wholesale fork of the ~2.5-million-line VS Code codebase on day one—which creates crushing upstream rebase debt, slows down CI, and impedes distribution—JAGGU adopts a **Dual-Ring Architecture**:
1. **Ring 0 (JAGGU Core Extension & Agent Daemon)**: Ships 75–85% of full agent capabilities (context engine, agent orchestrator, model gateway, secure tool runtime, diff review view, and interactive planning UI) as a decoupled TypeScript/Node.js engine leveraging VS Code's rich Extension Host APIs, Custom Editor / Webview APIs, Language Server Protocol (LSP), and Terminal APIs.
...
| Layer | Source Directory | Responsibility | UI vs Node | JAGGU Relevance |
|---|---|---|---|---|
...
| **Server / Remote** | `src/vs/server/` | Headless remote execution environment for web and containerized development. | Node.js | Enables JAGGU headless agent execution in remote/containerized environments. |
...
- **Implication for JAGGU**: Heavy AI operations (AST parsing, ripgrep code traversal, vector calculation, LLM token streaming parsing) must live either in the Extension Host or in an auxiliary worker thread/daemon to prevent frame drops in the editor UI.
...
- **Pseudoterminal API**: VS Code exposes `ExtensionTerminalOptions` with `Pseudoterminal`. This allows JAGGU to construct a synthetic, fully controlled terminal pipe (`onDidWrite`, `onDidClose`, `handleInput`).
- **Headless Command Execution**: For background autonomous test execution and compilation error capturing, JAGGU uses Node's `child_process.spawn` with pty emulation (`node-pty`) directly inside the agent runtime, outputting structured traces to an interactive pseudo-terminal view.
...
- **JAGGU Advantage**: Allows non-destructive draft generation. The agent computes diffs in-memory, presents a unified Myers diff preview to the developer with per-hunk "Accept / Reject" action buttons, and flushes to disk only on developer confirmation.
...
- **Diagnostics**: `vscode.languages.getDiagnostics()` extracts compiler errors, linter violations, and syntax warnings in real time, serving as the sensory input for JAGGU's self-healing debugging loop.
...
- **JAGGU Role**: Inspects working tree changes before and after task execution, computes rollbacks via `git checkout` / `git stash`, and assists with semantic commit messages.
...
jaggu-workspace/
├── packages/
│   ├── jaggu-core/           # Platform-agnostic Agent engine (state machine, tools, context, model gateway)
│   ├── jaggu-ui/             # Webview React/Tailwind UI (Activity bar, Sidebar, Plan Cards, Approvals)
│   ├── jaggu-vscode/         # VS Code Extension adapter (Commands, TextDocumentContentProvider, LSP, Git)
│   └── jaggu-eval/           # Benchmarking & SWE-bench style evaluation harness
