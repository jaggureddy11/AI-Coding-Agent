...
To ensure production-grade reliability across all ForgeAI components, testing is organized into four distinct tiers:
...
### 2.1 Agent State Machine Tests (`packages/forgeai-core/test/agent`)
...
### 2.2 Tool Schema & Argument Validation Tests (`packages/forgeai-core/test/tools`)
...
### 2.3 Context Priority & Token Trimming Tests (`packages/forgeai-context/test`)
...
### 2.4 Diff Engine Tests (`packages/forgeai-core/test/diff`)
...
- Agent requests `run_command` with `echo "ForgeAI Test"`.
...
2. Activates the ForgeAI extension (`packages/forgeai-vscode`).
...
ForgeAI includes an automated evaluation harness (`packages/forgeai-eval`) inspired by SWE-bench:
